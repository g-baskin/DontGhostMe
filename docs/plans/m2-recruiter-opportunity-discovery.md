...
Create a small fictional `.example` labeled corpus covering human and agency recruiters, internal recruiters, non-recruiter mail, alerts, newsletters, ATS acknowledgements, calendar notices, receipts, malformed input, aliases, shared inboxes, same-name people, company changes, opportunity ambiguity, explicit submissions, negative submission examples, silence, and unknown outcomes.
...
Run focused classification, migration, persistence, API/component, and Playwright checks first. Then run `git diff --check`, frozen Bun 1.4 install, `bun run check`, `bun run typecheck`, `bun run test`, `bun run test:export`, `bun run build`, `bun run test:e2e`, `bun run db:backup`, and foreign-key verification. Treat unavailable Node 24/26 matrix execution as the already accepted soft follow-up.

## Steps

1. Create this plan and update roadmap status without changing M1 boundaries.
2. Add typed owner-identity, classification-run, proposal, evidence, decision, signal, and outcome contracts.
3. Add migration `0002`, synchronized schema/snapshot metadata, export/backup coverage, and deletion order.
4. Implement pure deterministic proposal rules with versioned signal weights.
5. Add the fictional labeled evaluation corpus and pure-rule threshold tests.
6. Implement owner-scoped repositories, checkpoint persistence, decisions, and version-preserving reprocessing.
7. Implement transactional proposal promotion into canonical M0 records.
8. Add localhost-guarded owner-alias, run/resume, proposal, decision, merge, and split APIs.
9. Expand the review queue with explainable, correctable, accessible proposal workflows.
10. Add migration, lifecycle, isolation, redaction, preservation, export, backup, deletion, component, and E2E tests.
11. Update product documentation, ADRs, and the compliance register.
12. Run security review, quality audit, and the full verification sequence before closeout.
